"""Reproduce the JMN1 observer-sky image corresponding to Figure 2(d)."""
...
from src.raytracing import ImageConfig, build_jmn1_image
from src.shadow import jmn1_shadow
...
def main() -> None:
    ...
    x_axis, y_axis, image = build_jmn1_image(spacetime, rt_config, image_config)
    shadow = jmn1_shadow(spacetime)
    ...
    ax.set_xlabel(r"$X$")
    ax.set_ylabel(r"$Y$")
    ax.set_title(r"JMN1 observer-sky image for $M_0=0.7$ (paper Fig.~2(d))")
    cbar = fig.colorbar(im, ax=ax, shrink=0.88)
    cbar.set_label(r"$I_{\rm o}(X,Y)$")

    output = os.path.join(PROJECT_ROOT, "figures", "figure2d_jmn1_shadow.png")
